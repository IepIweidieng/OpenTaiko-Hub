<script>
	...
	import songSubmissionScreenshotUrl from '$lib/songsubmission.jpg';
	import songSuggestionScreenshotUrl from '$lib/songsuggestion.jpg';
...
{#if currentTab === 0}
	<div class="content">
		...
		<div class="w-full text-token grid grid-cols-2 md:grid-cols-2 gap-4">
			<div class="card bg-initial card-hover overflow-hidden">
				...
				<div class="p-4 space-y-4 text-black dark:text-white">
					<h2 data-toc-ignore="">Taiko .TJA Charting Tutorial</h2>
		...
		{#if optk_OS === "Win"}
			<h1 class="my-3">Download PeepoDrumKit:</h1>
			<div class="w-full text-token grid grid-cols-2 md:grid-cols-2 gap-4">	
				<ToolCard 
...
{#if currentTab === 2}
	<div class="content">
		<h1 class="my-3">OpenTaiko soundtrack</h1>
		<div class="w-full text-token grid grid-cols-1 md:grid-cols-2 gap-4">
			<ToolCard 
